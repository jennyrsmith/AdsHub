...
export async function refreshDailyRollup(startDate, endDate) {
  ...
  try {
    await queryWithRetry(sql, [start, end]);
...
export async function refreshMvSummary30d() {
  try {
    await queryWithRetry('REFRESH MATERIALIZED VIEW CONCURRENTLY mv_summary_30d');
  } catch (err) {
    if (err.code === '55000') {
      await queryWithRetry('REFRESH MATERIALIZED VIEW mv_summary_30d');
    } else {
      throw err;
    }
  }
